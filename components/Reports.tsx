...
const Reports: React.FC<ReportsProps> = ({ data }) => {
  ...
  const classData = useMemo(() => {
    ...
    const records = data.records.filter(r => r.classId === selectedClassId);
    ...
    sortedRecords.forEach(r => {
        ...
        groupedByDate[r.date] = { date: r.date, score: runningScore / (students.length || 1) };
    });
      ...
      <div className="flex flex-wrap justify-between items-end gap-3">
       ...
       <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex flex-wrap items-center justify-between gap-6">
            <div className="flex items-center gap-2">
                <button className="px-5 py-2 rounded-lg text-sm font-bold bg-primary text-white border border-primary shadow-sm">Tuần này</button>
                <button className="px-5 py-2 rounded-lg text-sm font-medium text-slate-500 hover:bg-slate-100 transition-colors">Tháng này</button>
                <button className="px-5 py-2 rounded-lg text-sm font-medium text-slate-500 hover:bg-slate-100 transition-colors">Học kỳ</button>
                <button className="px-5 py-2 rounded-lg text-sm font-medium text-slate-500 hover:bg-slate-100 transition-colors flex items-center gap-1">
